...
<form action="{{ route('store.careers.update', $career->id) }}" method="POST">
    ...
    <div class="panel-card">
        <div class="panel-card-header">
            <h2 class="panel-card-title">
                <i class="bi bi-briefcase"></i> Career Details
            </h2>
        </div>
    ...
    @php
        $existingSpecifications = $career->specifications->map(function ($spec) {
            return [
                'id' => $spec->id,
                'name' => $spec->name,
                'type' => $spec->type,
    ...
    @push('scripts')
        <script>
            window.__existingSpecifications = @json($existingSpecifications);
        </script>
    @endpush
